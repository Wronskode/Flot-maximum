...
\hypersetup{
	pdftitle={Résolution du problème du flot maximum}
}
\usepackage{datetime}
\usepackage{numprint}
\usepackage{listings}
...
\newdate{frontpagedate}{20}{05}{2025} 
\begin{document}
...
L’algorithme de Ford-Fulkerson a été introduit en 1956 par L. R. Ford Jr. et D. R. Fulkerson. Il s’agit d’un des premiers algorithmes systématiques permettant de résoudre le problème du flot maximal dans un réseau de transport. Ce problème consiste à envoyer le maximum de « flot » d’un sommet source vers un sommet puits dans un graphe orienté, tout en respectant des capacités sur les arêtes.

Le principe fondamental de Ford-Fulkerson repose sur l’idée intuitive suivante : tant qu’il existe un chemin dans le graphe où du flot peut encore être envoyé (appelé \textit{chemin augmentant}), alors on peut accroître le flot total.
...
\end{algorithm}
\begin{definition}
	Le graphe résiduel est une structure clé de l’algorithme. Pour chaque arête $(u,v)$ de capacité $c(u,v)$ et flot actuel $f(u,v)$, on définit une capacité résiduelle $c_f(u,v) = c(u,v) - f(u,v)$. De plus, une arête inverse $(v,u)$ est introduite avec capacité résiduelle $f(u,v)$, permettant d’annuler du flot si nécessaire.
\end{definition}
\subsubsection{Complexité}
La complexité de l’algorithme dépend du choix des chemins augmentants :
\begin{itemize}
    \item Si on choisit les chemins de manière arbitraire (DFS par exemple), la complexité est $\mathcal{O}(F \cdot E)$, où $F$ est la valeur du flot maximal (supposée entière) et $E$ le nombre d’arêtes.
    \item En l’absence de capacité entière, l’algorithme peut ne jamais converger (phénomène d’oscillation du flot par petits incréments).
\end{itemize}
...
\textbf{Inconvénients :}
\begin{itemize}
    \item Pas de borne de complexité polynomiale sans restriction sur les données mais il peut aussi être lent en pratique à cause du choix des chemins et il est possible qu'il ne se termine jamais si les capacités ne sont pas entières.
\end{itemize}
...
\subsubsection{Complexité}
Contrairement à Ford-Fulkerson, Edmonds-Karp garantit une borne polynomiale. La clé est que le nombre de fois qu’une arête peut être saturée est borné.
...
\textbf{Inconvénients :}
\begin{itemize}
    \item Moins efficace que des algorithmes plus avancés comme Dinic ou Poussage-Réétiquetage sur les grands graphes.Il peut aussieffectuer un nombre élevé d’itérations si le graphe contient beaucoup de chemins courts.
\end{itemize}
\subsubsection{Application sur l'exemple}
...
\begin{itemize}
    \item \textbf{Cas général} : $\mathcal{O}(V^2 E)$. Il y a au plus $V - 1$ niveaux possibles, donc au plus $V$ phases. Chaque phase utilise un BFS en $\mathcal{O}(E)$ et une recherche de flot bloquant pouvant prendre $\mathcal{O}(E)$ DFS. D’où un coût de $\mathcal{O}(E^2)$ par phase, et $\mathcal{O}(V \cdot E^2)$ au total. Mais cette borne est souvent pessimiste.
\end{itemize}

\subsubsection{Avantages/inconvénients}
\textbf{Avantages :}
\begin{itemize}
    \item Meilleure complexité asymptotique que Ford-Fulkerson et Edmonds-Karp dans de nombreux cas.
    \item Exploite des structures efficaces (graphe de niveaux, DFS optimisé).
    \item Permet un gain significatif en pratique sur des grands graphes.
...
\begin{itemize}
    \item \colorbox{gray!20}{\textcolor{violet}{Vertex}}
    \item \colorbox{gray!20}{\textcolor{violet}{Graph}}
    \item \colorbox{gray!20}{\textcolor{violet}{FlowNetwork}}
    \item \colorbox{gray!20}{\textcolor{violet}{FileFlowNetwork}}
    \item \colorbox{gray!20}{\textcolor{violet}{RandomFlowNetwork}}
    \item \colorbox{gray!20}{\textcolor{violet}{PL}}
    \item \colorbox{gray!20}{\textcolor{violet}{Curve}}
    \item \colorbox{gray!20}{\textcolor{violet}{Program}}
\end{itemize}
...
Quelques fonctions de bases sont codées, comme par exemple \textcolor{green!30!black}{NeighborsLeft} où \textcolor{green!30!black}{NeighborsRight}, qui retournent respectivement les voisins entrants et sortants d'un sommet donné, une fonction \textcolor{green!30!black}{Clone} qui clone le graphe et d'autres fonctionnalités classiques.
...
\begin{itemize}
	\item \textcolor{green!30!black}{FordFulkerson}
	\item \textcolor{green!30!black}{EdmondsKarp}
	\item \textcolor{green!30!black}{Dinic}
	\item \textcolor{green!30!black}{PushRelabel}
\end{itemize}
...
\begin{itemize}
	\item \textcolor{green!30!black}{DFS\_GetPathInLevelGraph} : implémente une recherche en profondeur (DFS) pour trouver un chemin augmentant dans un graphe résiduel (typiquement dans l'algorithme de FordFulkerson ou de Dinic).
	\item \textcolor{green!30!black}{BFS\_GetLevels} : utilise une recherche en largeur (BFS) pour attribuer à chaque sommet son niveau (distance minimale en nombre d’arêtes depuis la source) dans un graphe.
	\item \textcolor{green!30!black}{GetResidualGraph} : permet de récupérer le graphe résiduel du réseau de flot.
	\item \textcolor{green!30!black}{GetMaxFlow} : permet de récupérer la valeur du flot maximum. L'interêt et les propriétés d'un tel graphes sont détaillés dans la section sur les algorithmes de flot maximum.
	\item \textcolor{green!30!black}{IsConnected} : cette méthode n'est pas utilisée dans les algorithmes de flot maximum, mais elle a été très utile lors de la création des instances. En effet, elle permet de vérifier plusieurs propriétés sur le graphe qui sont essentielles pour s'assurer que le graphe est \og{}bien formé\fg{}. Comme détaillé dans la partie sur les instances, elle vérifie que tout sommet est accessible depuis la source et que tout sommet admet un chemin jusqu'au puits. Elle décide donc si un graphe est gardé comme instance ou jeté.
	\item \textcolor{green!30!black}{CreateGraphFile} : permet de créer un fichier texte contenant la représentation du graphe. Cette méthode est utilisée lors de la création des instances, afin de pouvoir les sauvegarder et les réutiliser plus tard.
\end{itemize}
...
L’algorithme proposé par \textbf{Bernstein, Liu, Nanongkai, Probst Gutenberg, Sidford, Spinrad, Sun et Wang} en 2020 marque une percée dans l’optimisation du flot maximal. Il atteint une complexité de :
\[
...
L’approche consiste à ramener le flot maximum à un problème de minimisation de potentiel dans un graphe résiduel, résolu par une succession de systèmes linéaires approchés. Les résultats atteignent des complexités du type :
\[
...
\subsection{Travaux récents de Bernstein, Blikstad, Saranurak, Tu (2023)}

En 2023, une nouvelle avancée est publiée par \textbf{Bernstein, Blikstad, Saranurak et Tu} qui propose un algorithme atteignant :
\[
\mathcal{O}\left(n^{2 + o(1)} \log U\right)
\]
pour des classes spécifiques de graphes, notamment les \textit{bounded genus graphs} ou ceux admettant des séparateurs efficaces.

Leur travail combine :
\begin{itemize}
    \item des techniques de \textit{vertex sparsification},
    \item la décomposition récursive des graphes,
    \item la réutilisation de flots partiels,
    \item et une architecture algorithmique à base d’oracles dynamiques.
\end{itemize}

Il s’agit d’une des tentatives les plus abouties pour traiter le flot maximum dans des contextes très particuliers, tout en garantissant une efficacité presque linéaire.
...
Dans le cadre de ce TER, nous nous sommes concentrés sur l’algorithme de Dinic pour sa valeur pédagogique et son efficacité structurelle, mais il est important de souligner que la recherche algorithmique continue de produire des méthodes de plus en plus puissantes, qui pourraient bientôt devenir des standards pratiques.
