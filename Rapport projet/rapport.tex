...
Un chemin améliorant peut être trouvé en $O(|A|)$ avec une recherche en profondeur ou en largeur par exemple, et un tel chemin augmente le flot d'au moins une unité, et d'au plus $|f^\star|$, où $f^\star$ représente un flot maximum. En notant $n$ le nombre de sommets et $m$ le nombre d'arcs, on a donc une complexité de $O(m|f^\star|)$ pour l'algorithme de Ford-Fulkerson.
...
\subsubsection{Complexité}
La complexité de l'algorithme d'Edmonds-Karp est de $O(|A|^2|V|)$.
...
\begin{table}[H]
	...
	\begin{tabular}{|c|c|c|}
		\hline
		Algorithme & Complexité & Complexité en fonction de $n$ \\
		\hline
